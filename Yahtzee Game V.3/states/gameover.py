...
class GameOver(BaseState):
    ...
    def draw(self, surface):
        #title screen image
        titleScreen = pygame.image.load('yahtzee-gameover-screen.png')
        ...
        if (self.persist[3] > self.lowestScore):
            self.highScore = True
            highScoreText = textFont.render("New Highscore! Please enter your name! (press enter to submit)", True, pygame.Color("Blue"))
            surface.blit(highScoreText, (100, 250))
            
            nameText = textFont.render(self.name, True, pygame.Color("Blue"))
            surface.blit(nameText, (10, 270))

            nameField = textFont.render("___________________________________", True, (0))
            surface.blit(nameField, (10, 270))
        ##DON'T FORGET TO RESET SELF PERSIST WHEN THE GAME IS OVER!##
