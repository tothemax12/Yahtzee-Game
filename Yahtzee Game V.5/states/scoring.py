...
class Scoring(BaseState):
    ...
    def get_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            scoreClicked = yahtzeeGraphics.displayPossibleScores(scoreCheck.getCalculatedScores(scoreCheck.getChoices(self.persist[0]), self.persist[0], self.yahtzeeCount), pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[1], True, self.persist[4])
            print(scoreClicked)
            if (scoreClicked != -1):
                ...
                if (not scoreCheck.checkIfScoreWasPickedAlready((self.persist[4][scoreClicked[1]]), clickedYahtzee)):
                     if (clickedYahtzee and not self.yahtzeeIsZero):#if yahtzee was clicked and there is not a zero in the box
                        self.yahtzeeCount += 1 #keep track of valid yahtzees
                        ...
                        #Add score to total score. First Yahtzee gets a score of 50, subsequent yahtzees score 100
                        #if self.yahtzeeCount == 1:
                        #    self.persist[3] += 50
                        #else:
                        #    self.persist[3] += 100
                        ...
                        if (not gameOver):
                            ...
                            self.done = True #end current state
                     elif not clickedYahtzee: #if yahtzee was not clicked at all
                        #reset the held indeces
                        self.persist[1] = [-1, -1, -1, -1, -1]
                        #don't reset the counter if player clicks a zero score like a little cheapskate (update I found out 0 is a valid and important score in Yahtzee.)
                        #if (scoreClicked[0] == -1):
    ...
    def draw(self, surface):
        ...
        validScores = scoreCheck.getCalculatedScores(scoreCheck.getChoices(self.persist[0]), self.persist[0], self.yahtzeeCount)
        
        #keep track of if Yahtzee is a 0, if it is the player can't click it
        if (validScores[9] != 0):
            self.yahtzeeIsZero = False
        else:
            self.yahtzeeIsZero = True        
